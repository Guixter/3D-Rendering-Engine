...
public class Matrix {
    ...
    /**
     * Creates an empty matrix of size @rows x @cols
     */
     ...
     */ 
    public void scalarMultiply(double lambda) {
	for (int i = 0 ; i < nRows ; i++) {
	    for (int j = 0 ; j < nCols ; j++) {
		set(i, j, get(i, j) * lambda);
	    }
	}
    }
     ...
     */ 
    public static Matrix vecToMat(Vector v) {
	Matrix M;
	try {
    ...
    /**
     * Concat two matrixes horizontally
     */ 
    public static Matrix horConcat(Matrix A, Matrix B) {
	Matrix C;
	try {
	    C = new Matrix(A.nRows, A.nCols + B.nCols);
	} catch (InstantiationException e) {
	    return null;
	}
    ...
    /**
     * Concat two matrixes vertically
     */ 
    public static Matrix verConcat(Matrix A, Matrix B) {
	Matrix C;
	try {
	    C = new Matrix(A.nRows + B.nRows, A.nCols);
	} catch (InstantiationException e) {
	    return null;
	}
	for (int j = 0 ; j < A.nCols ; j++) {

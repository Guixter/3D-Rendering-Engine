...
public class Renderer {
    ...
    static void renderSolidPhong () {
		...
		for (int i = 0; i < 3 * mesh.getNumFaces(); i += 3) {
		    ...
		    Fragment v2 = fragments[faces[i+1]];
		    Fragment v3 = fragments[faces[i+2]];
		    Vector3 pos1 = new Vector3(vertices[faces[i]]);
		    Vector3 pos2 = new Vector3(vertices[faces[i+1]]);
		    Vector3 pos3 = new Vector3(vertices[faces[i+2]]);

		    rasterizer.rasterizeFacePhong (v1, v2, v3, pos1, pos2, pos3, scene, lighting);
		}
        } catch (Exception e) {}
    }
